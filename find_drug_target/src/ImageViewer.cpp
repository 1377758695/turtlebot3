...
#define WID_LEN_MIN 0.90 //(1.83, 1.30, 0.90, 0.84)
#define WID_LEN_MAX 1.00 //(2.03, 1.50, 1.00, 1.04)
#define TARGETS 8
#define DIST 50
using namespace std;
using namespace cv;
...
void ImageViewer::matchTarget()
{
    ...
    if(!objectImg.empty() && !sceneImg.empty())
	{
		...
		if(mpts_1.size() >= minInliers)
		{
			...
			// Change area to width/height, because area parameter will be influenced by different distance while width/height won't
			...
			{
				targetFindedTimes++;
				if ( (stop == false && targetFindedTimes >= TARGETS) || stop == true)
				{
...
int main(int argc, char **argv)
{
	...
	//myturtlebot3::ImageViewer viewer;
	ros::NodeHandle nh;
	ros::Subscriber subscriber = nh.subscribe("find_target", 1, callBack);    
    ros::spin();
